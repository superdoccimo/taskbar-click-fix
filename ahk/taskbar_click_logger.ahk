#Requires AutoHotkey v2.0
#SingleInstance Force

; Observation-only MVP for Windows 11 taskbar click issues.
; Logs left-click down/up timing, movement, target root class, and foreground change.

global LOG_PATH := A_ScriptDir "\..\taskbar_click_log.csv"
global gHook := 0
global gCb := 0

global gDownTick := 0
global gDownX := 0
global gDownY := 0
global gFgBefore := 0

EnsureLogHeader()
StartHook()
OnExit(StopHook)
Persistent

EnsureLogHeader() {
    global LOG_PATH
    if !FileExist(LOG_PATH)
        FileAppend("t_down,t_up,duration_ms,move_px,root_class,fg_changed`n", LOG_PATH, "UTF-8")
}

StartHook() {
    global gHook, gCb
    gCb := CallbackCreate(LowLevelMouseProc, , 3)
    gHook := DllCall("user32\SetWindowsHookExW"
        , "Int", 14
        , "Ptr", gCb
        , "Ptr", DllCall("kernel32\GetModuleHandleW", "Ptr", 0, "Ptr")
        , "UInt", 0
        , "Ptr")
    if !gHook
        MsgBox "SetWindowsHookEx failed. A_LastError=" A_LastError
}

StopHook(*) {
    global gHook, gCb
    if gHook
        DllCall("user32\UnhookWindowsHookEx", "Ptr", gHook)
    if gCb
        CallbackFree(gCb)
}

GetRootWindowClassAt(x, y) {
    hwnd := DllCall("user32\WindowFromPoint", "Int64", (y << 32) | (x & 0xFFFFFFFF), "Ptr")
    if !hwnd
        return ""
    hRoot := DllCall("user32\GetAncestor", "Ptr", hwnd, "UInt", 2, "Ptr")
    if !hRoot
        hRoot := hwnd
    buf := Buffer(256 * 2, 0)
    n := DllCall("user32\GetClassNameW", "Ptr", hRoot, "Ptr", buf, "Int", 256, "Int")
    return (n > 0) ? StrGet(buf, n, "UTF-16") : ""
}

GetForegroundHwnd() {
    return DllCall("user32\GetForegroundWindow", "Ptr")
}

CsvEscape(s) {
    if InStr(s, ",") || InStr(s, '"') {
        s := StrReplace(s, '"', '""')
        return '"' s '"'
    }
    return s
}

LowLevelMouseProc(nCode, wParam, lParam) {
    static WM_LBUTTONDOWN := 0x0201
    static WM_LBUTTONUP := 0x0202
    static LLMHF_INJECTED := 0x00000001

    if (nCode < 0)
        return DllCall("user32\CallNextHookEx", "Ptr", 0, "Int", nCode, "Ptr", wParam, "Ptr", lParam, "Ptr")

    x := NumGet(lParam, 0, "Int")
    y := NumGet(lParam, 4, "Int")
    flags := NumGet(lParam, 12, "UInt")
    injected := (flags & LLMHF_INJECTED) != 0
    if injected
        return DllCall("user32\CallNextHookEx", "Ptr", 0, "Int", nCode, "Ptr", wParam, "Ptr", lParam, "Ptr")

    global gDownTick, gDownX, gDownY, gFgBefore, LOG_PATH

    if (wParam = WM_LBUTTONDOWN) {
        gDownTick := A_TickCount
        gDownX := x
        gDownY := y
        gFgBefore := GetForegroundHwnd()
        return DllCall("user32\CallNextHookEx", "Ptr", 0, "Int", nCode, "Ptr", wParam, "Ptr", lParam, "Ptr")
    }

    if (wParam = WM_LBUTTONUP && gDownTick > 0) {
        tUp := A_TickCount
        duration := tUp - gDownTick
        dx := x - gDownX
        dy := y - gDownY
        move := Round(Sqrt(dx*dx + dy*dy))
        rootClass := GetRootWindowClassAt(x, y)
        fgAfter := GetForegroundHwnd()
        fgChanged := (fgAfter != gFgBefore)
        FileAppend(A_Now "," A_Now "," duration "," move "," CsvEscape(rootClass) "," fgChanged "`n", LOG_PATH, "UTF-8")
        gDownTick := 0
    }

    return DllCall("user32\CallNextHookEx", "Ptr", 0, "Int", nCode, "Ptr", wParam, "Ptr", lParam, "Ptr")
}
